...
/* ==================== CAF ==================== */
export function loadCAF(tipo) {
    const b64 = tipo === 39 ? process.env.CAF_39_B64 : process.env.CAF_41_B64;
    let xml;
    if (b64 && b64.trim())
        xml = Buffer.from(b64, "base64").toString("latin1");
    else {
        const p = tipo === 39 ? process.env.CAF_39_PATH : process.env.CAF_41_PATH;
        if (!p)
            throw new Error(`CAF_${tipo}_B64/CAF_${tipo}_PATH no configurado`);
...
function signDDwithRSASK(ddXml, rsaskPem) {
...
function forcePlainKeyInfo(signedXml, certB64) {
    let xml = signedXml
    ...
    if (/(<\/ds:Signature>\s*)$/i.test(xml))
        return xml.replace(/(<\/ds:Signature>\s*)$/i, `${keyInfo}$1`);
    if (/(<\/Signature>\s*)$/i.test(xml))
        return xml.replace(/(<\/Signature>\s*)$/i, `${keyInfo}$1`);
    return xml.replace(/(<\/(?:ds:)?SignatureValue>\s*)/i, `$1${keyInfo}`);
}
function signXmlEnveloped(xml, idToSign) {
    ...
    sig.keyInfoProvider = {
        getKeyInfo: () => `<ds:X509Data xmlns:ds="http://www.w3.org/2000/09/xmldsig#">
    ...
    return forcePlainKeyInfo(sig.getSignedXml(), certB64);
}
function signDocumento(dteXml) {
    const m = dteXml.match(/<Documento[^>]*\bID="([^"]+)"/i);
    if (!m)
        throw new Error("Documento sin ID");
    return signXmlEnveloped(dteXml, m[1]);
}
export function stampDTEWithCAF(dteXml, caf) {
...
export async function getSeed() {
    ...
    if (!ret?.textContent)
        throw new Error(`No <getSeedReturn>. Head: ${resp.slice(0, 200)}`);
    const decoded = unescapeXml(ret.textContent);
    const m = decoded.match(/<SEMILLA>([^<]+)<\/SEMILLA>/i);
    if (!m)
        throw new Error(`No <SEMILLA>. Head: ${decoded.slice(0, 200)}`);
    return m[1].trim();
}
function buildSeedXML(seed) {
...
async function getTokenFromSeed(signedXml) {
    ...
    if (!ret?.textContent)
        throw new Error(`No <getTokenReturn>. Head: ${resp.slice(0, 200)}`);
    const decoded = unescapeXml(ret.textContent);
    const m = decoded.match(/<TOKEN>([^<]+)<\/TOKEN>/i);
    if (!m)
        throw new Error(`No <TOKEN>. Head: ${decoded.slice(0, 200)}`);
    return m[1].trim();
}
export async function getToken() {
    if (!hasClientCert() || !process.env.SII_CERT_PASSWORD)
...
function extractTrackIdFromUpload(respSoap) {
    ...
    if (!ret?.textContent)
        throw new Error(`No <uploadReturn>. Head: ${respSoap.slice(0, 400)}`);
